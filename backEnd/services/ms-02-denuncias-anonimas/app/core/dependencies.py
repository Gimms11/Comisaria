...
async def get_current_officer(
    ...
    """Valida el JWT del oficial emitido por MS-01."""
    if not credentials:
        raise HTTPException(
...
def require_roles(*allowed_roles: OfficerRole) -> Callable:
    """Control de acceso por roles para endpoints policiales."""
    async def role_checker(
        ...
        if current_officer.role not in allowed_roles:
            raise HTTPException(
                ...
                detail="Permiso denegado para esta acción policial",
            )
